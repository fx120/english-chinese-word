...
class Index extends Controller
{
    ...
    public function params()
    {
        ...
        if ($chunk) {
            $options = [
                ...
                'provider'         => new StaticCredentialsProvider($config['accessKeyId'], $config['accessKeySecret']),
                'signatureVersion' => OssClient::OSS_SIGNATURE_VERSION_V4
            ];
            $oss = new OssClient($options);
        ...
        if ($chunkid) {
            ...
            if ($action == 'merge') {
                ...
                if (!isset($result['Key'])) {
                    ...
                    $data = $attachment && method_exists($attachment, 'getBaseData') ? $attachment->getBaseData() : ['url' => "/" . $key, 'fullurl' => cdnurl("/" . $key, true)];
            ...
            unset($data['id']);
            $data['storage'] = 'alioss';
            $attachment = Attachment::create($data, true);

            $this->success("上传成功", '', method_exists($attachment, 'getBaseData') ? $attachment->getBaseData() : ['url' => $url, 'fullurl' => cdnurl($url, true)]);
        }
        return;
    }
    ...
    public function notify()
    {
        ...
        $data = $attachment && method_exists($attachment, 'getBaseData') ? $attachment->getBaseData() : ['url' => $url, 'fullurl' => cdnurl($url, true)];
