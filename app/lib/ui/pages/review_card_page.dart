...
/// 题型
enum ReviewQuizType { enToCn, cnToEn, fillLetters }

/// 复习卡片页面
///
/// 流程：直接出题 → 答对下一个 → 答错显示完整释义/发音
...
class _ReviewCardPageState extends State<ReviewCardPage> {
  ...
  ReviewQuizType _quizType = ReviewQuizType.enToCn;
  ...
  void dispose() {
    ...
    if (_session != null) {
      try { context.read<ReviewProvider>().reviewManager.endReviewSession(_session!); } catch (_) {}
    }
    super.dispose();
  }
  ...
  Future<void> _startReviewSession() async {
    ...
    try {
      final rm = context.read<ReviewProvider>().reviewManager;
      final session = await rm.startReviewSession(widget.vocabularyList.id, widget.mode);
      if (mounted) {
        setState(() { _session = session; _totalWords = session.reviewWordIds.length; });
        await _loadNextWord();
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _loadNextWord() async {
    ...
    try {
      final rm = context.read<ReviewProvider>().reviewManager;
      final word = await rm.getNextReviewWord(_session!);
      if (mounted) {
        if (word == null) {
  ...
  Future<void> _prepareQuiz() async {
    if (_currentWord == null) return;
    final types = ReviewQuizType.values;
    ...
    switch (_quizType) {
      ...
      case ReviewQuizType.cnToEn:
        ...
        break;
      case ReviewQuizType.fillLetters:
  ...
  void _prepareFillLetters(List<Word> wrongWords) {
    ...
    if (word.length < 3) {
      _quizType = ReviewQuizType.enToCn;
      final options = wrongWords.map((w) => w.definition).toList();
      options.add(_currentWord!.definition);
      options.shuffle(_random);
      _choices = options;
    ...
    // 策略2：常见易混淆字母替换
    const confusables = {
      'a': ['e', 'o', 'u'], 'e': ['a', 'i', 'o'], 'i': ['e', 'y', 'a'],
      'o': ['u', 'a', 'e'], 'u': ['o', 'a', 'i'], 'b': ['d', 'p'],
      'd': ['b', 't'], 'p': ['b', 'q'], 'q': ['p', 'g'], 'g': ['q', 'j'],
      'c': ['k', 's'], 'k': ['c', 'g'], 's': ['c', 'z'], 'z': ['s', 'x'],
      'n': ['m', 'u'], 'm': ['n', 'w'], 't': ['d', 'f'], 'f': ['v', 't'],
      'v': ['f', 'w'], 'w': ['v', 'm'], 'l': ['r', 'i'], 'r': ['l', 'n'],
    };
  ...
  void _onSelectChoice(int index) {
    ...
    setState(() { _selectedChoiceIndex = index; _isAnswerCorrect = correct; });
    _recordAnswer(correct);
    if (correct) {
      Future.delayed(const Duration(milliseconds: 600), () { if (mounted) _loadNextWord(); });
    } else {
  ...
  Future<void> _recordAnswer(bool correct) async {
    ...
    try {
      final rm = context.read<ReviewProvider>().reviewManager;
      if (correct) {
        await rm.markWordAsRemembered(_currentWord!.id, widget.vocabularyList.id);
      } else {
        await rm.markWordAsForgotten(_currentWord!.id, widget.vocabularyList.id);
      }
    } catch (_) {}
  }
  ...
  Future<void> _showCompletionStats() async {
    ...
    try {
      final rm = context.read<ReviewProvider>().reviewManager;
      final stats = await rm.endReviewSession(_session!);
      _session = null;

      // 更新统计数据
      try {
        final sp = context.read<StatisticsProvider>();
        await sp.statisticsManager.incrementTodayReviewWords(stats.totalWordsReviewed);
        ...
        await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: Row(children: [
              ...
              const Text('复习完成！', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ]),
            content: Column(mainAxisSize: MainAxisSize.min, children: [
              _statRow(Icons.library_books, Colors.blue, '复习单词', '${stats.totalWordsReviewed}'),
              const SizedBox(height: 10),
              _statRow(Icons.check_circle, Colors.green, '答对', '${stats.rememberedWordsCount}'),
              const SizedBox(height: 10),
              _statRow(Icons.error, Colors.orange, '答错', '${stats.forgottenWordsCount}'),
              const SizedBox(height: 10),
  ...
  Widget build(BuildContext context) {
    final modeTitle = widget.mode == ReviewMode.memoryCurve ? '记忆曲线复习' : '错题复习';
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, result) async {
        ...
        if (shouldPop == true && mounted) {
          if (_session != null) {
            try { context.read<ReviewProvider>().reviewManager.endReviewSession(_session!); _session = null; } catch (_) {}
          }
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(modeTitle), backgroundColor: Colors.purple, foregroundColor: Colors.white,
          actions: [
        ...
        body: _isLoading
            ...
            : _currentWord == null
                ? const Center(child: Text('没有可复习的单词'))
                : Column(children: [
  ...
  Widget _buildSessionStats() {
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          Icon(Icons.trending_up, color: Colors.purple.shade600, size: 18),
          const SizedBox(width: 6),
          Text('复习进度 $_currentIndex/$_totalWords', style: TextStyle(fontSize: 13, color: Colors.grey.shade700)),
          const Spacer(),
          Container(
  ...
  Widget _buildQuiz() {
    switch (_quizType) {
      case ReviewQuizType.enToCn: return _buildEnToCnQuiz();
      case ReviewQuizType.cnToEn: return _buildCnToEnQuiz();
      case ReviewQuizType.fillLetters: return _buildFillLettersQuiz();
    }
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Card(elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        child: Padding(padding: const EdgeInsets.symmetric(vertical: 32, horizontal: 24), child: Column(children: [
          Text(_currentWord!.word, style: const TextStyle(fontSize: 34, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
  ...
  Widget _buildCnToEnQuiz() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Card(elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        child: Padding(padding: const EdgeInsets.symmetric(vertical: 32, horizontal: 24), child: Column(children: [
          Text(maskedDef, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, height: 1.4), textAlign: TextAlign.center),
          if (_currentWord!.partOfSpeech != null && _currentWord!.partOfSpeech!.isNotEmpty) ...[
            const SizedBox(height: 10),
            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
              decoration: BoxDecoration(color: Colors.purple.shade50, borderRadius: BorderRadius.circular(8)),
              child: Text(_currentWord!.partOfSpeech!, style: TextStyle(fontSize: 13, color: Colors.purple.shade700))),
          ],
        ]))),
      const SizedBox(height: 24),
      ..._buildChoiceButtons(_choices, _correctChoiceIndex, _onSelectChoice),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Card(elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        child: Padding(padding: const EdgeInsets.symmetric(vertical: 28, horizontal: 24), child: Column(children: [
          Text(maskedDef, style: const TextStyle(fontSize: 18, height: 1.4), textAlign: TextAlign.center),
  ...
  Widget _quizHeader(String text) {
    return Container(
      ...
      child: Text(text, style: TextStyle(fontSize: 15, color: Colors.purple.shade800, fontWeight: FontWeight.w500), textAlign: TextAlign.center),
    );
  }
  ...
  List<Widget> _buildChoiceButtons(List<String> choices, int correctIndex, void Function(int) onSelect) {
    return List.generate(choices.length, (i) {
      ...
      if (_selectedChoiceIndex != null) {
        if (i == correctIndex) { bgColor = Colors.green.shade50; borderColor = Colors.green; textColor = Colors.green.shade800; }
        else if (i == _selectedChoiceIndex && i != correctIndex) { bgColor = Colors.red.shade50; borderColor = Colors.red; textColor = Colors.red.shade800; }
      }
      return Padding(padding: const EdgeInsets.only(bottom: 12), child: Material(
        ...
        child: InkWell(
          ...
          child: Padding(padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 16), child: Row(children: [
            Container(width: 32, height: 32, decoration: BoxDecoration(color: Colors.purple.shade50, borderRadius: BorderRadius.circular(8)),
              child: Center(child: Text(letter, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.purple.shade700)))),
            const SizedBox(width: 12),
            Expanded(child: Text(choices[i], style: TextStyle(fontSize: 16, color: textColor), maxLines: 3, overflow: TextOverflow.ellipsis)),
            if (_selectedChoiceIndex != null && i == correctIndex) const Icon(Icons.check_circle, color: Colors.green, size: 22),
            if (_selectedChoiceIndex != null && i == _selectedChoiceIndex && i != correctIndex) const Icon(Icons.cancel, color: Colors.red, size: 22),
          ])),
        ),
      ));
    });
      ...
      Card(
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            IconButton(onPressed: () => _playPronunciation(type: 1), icon: Icon(Icons.volume_up, color: Colors.purple.shade600), tooltip: '美音'),
